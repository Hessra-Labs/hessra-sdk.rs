...
pub enum TokenType {
    ...
    Identity,
    /// Authorization token - grants access to a resource
    Authorization,
}
...
impl fmt::Display for TokenType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            TokenType::Authorization => write!(f, "authorization"),
        }
    }
}
...
pub enum TokenStructure {
    ...
    ServiceChain { nodes: usize },
    /// Token with multi-party attestations
    MultiParty { parties: usize },
...
impl fmt::Display for TokenStructure {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            TokenStructure::ServiceChain { nodes } => write!(f, "service_chain(nodes={nodes})"),
            TokenStructure::MultiParty { parties } => write!(f, "multi_party(parties={parties})"),
...
pub enum BlockType {
    ...
    /// A service chain attestation block
    ServiceChainAttestation { service: String },
    /// A multi-party attestation block
    MultiPartyAttestation { namespace: String },
...
impl fmt::Display for BlockType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            BlockType::ServiceChainAttestation { service } => {
                write!(f, "service_chain_attestation(service={service})")
            }
            BlockType::MultiPartyAttestation { namespace } => {
                write!(f, "multi_party_attestation(namespace={namespace})")
            }
...
pub struct TokenClassification {
    ...
    /// Resource (for authorization tokens)
    pub resource: Option<String>,
    /// Operation (for authorization tokens)
...
/// - Token type (identity vs authorization)
/// - Token structure (base, delegated, service chain, etc.)
...
fn determine_token_type(
    ...
    let mut is_authorization = false;
    ...
    for line in content.lines() {
        ...
        // Look for authorization token markers
    ...
    let token_type = if is_identity {
        ...
        // Default to authorization if unclear
        TokenType::Authorization
    };
...
fn classify_block_type(block_content: &str) -> BlockType {
    // Look for common patterns in block content
    ...
    // Check for service chain attestation (service fact)
    if block_content.contains("service(") {
        if let Some(service) = extract_quoted_value(block_content, "service(") {
            return BlockType::ServiceChainAttestation { service };
        }
    }

    // Check for multi-party attestation (namespace fact)
    if block_content.contains("namespace(") {
        if let Some(namespace) = extract_quoted_value(block_content, "namespace(") {
            return BlockType::MultiPartyAttestation { namespace };
        }
    }
...
fn determine_structure(blocks: &[BlockMetadata]) -> TokenStructure {
    ...
    let mut has_service_chain = false;
    let mut service_chain_count = 0;
    let mut has_multi_party = false;
    let mut multi_party_count = 0;
    ...
    for block in blocks.iter().skip(1) {
        ...
        match &block.block_type {
            ...
            BlockType::ServiceChainAttestation { .. } => {
                has_service_chain = true;
                service_chain_count += 1;
            }
            BlockType::MultiPartyAttestation { .. } => {
                has_multi_party = true;
                multi_party_count += 1;
            }
    ...
    } else if has_service_chain {
        TokenStructure::ServiceChain {
            nodes: service_chain_count,
        }
    } else if has_multi_party {
        TokenStructure::MultiParty {
            parties: multi_party_count,
        }
...
mod tests {
    ...
    #[test]
    fn test_classify_base_authorization_token() {
        ...
        assert_eq!(classification.token_type, TokenType::Authorization);

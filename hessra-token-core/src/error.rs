...
pub enum TokenError {
    ...
    #[error("Identity hierarchy violation: actor '{actual}' is not authorized for identity '{expected}' (delegatable: {delegatable})")]
    HierarchyViolation {
    ...
    #[error("Authorization denied: subject '{subject}' does not have permission to perform '{operation}' on '{resource}'")]
    RightsDenied {
    ...
    #[error("Service chain verification failed for component '{component}': {reason}")]
    ServiceChainFailed {
    ...
    #[error("Multi-party attestation missing for component '{component}' (expected key: {expected_key})")]
...
impl std::fmt::Display for ServiceChainFailure {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ServiceChainFailure::ComponentNotFound => write!(f, "component not found in chain"),
            ServiceChainFailure::MissingAttestation => write!(f, "attestation missing"),
            ServiceChainFailure::InvalidAttestation => write!(f, "attestation signature invalid"),
            ServiceChainFailure::KeyMismatch { expected, actual } => {
                write!(f, "key mismatch (expected: {expected}, actual: {actual})")
            }
            ServiceChainFailure::Other(reason) => write!(f, "{reason}"),
        }
    }
}
...
impl TokenError {
    ...
    pub fn is_service_chain_failure(&self) -> bool {
        matches!(self, TokenError::ServiceChainFailed { .. })
    }
    ...
    pub fn get_denied_access(&self) -> Option<(&str, &str, &str)> {
        match self {
            TokenError::RightsDenied {
                ...
                resource,
                operation,
            } => Some((subject.as_str(), resource.as_str(), operation.as_str())),
            _ => None,
        }
    }

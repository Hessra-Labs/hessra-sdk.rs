...
mod tests {
    ...
    fn test_basic_identity_token_creation_and_verification() {
        ...
        assert!(
            verify_identity_token(token.clone(), public_key, "urn:hessra:alice:agent".to_string()).is_ok(),
    ...
    fn test_single_level_delegation() {
        ...
        let base_verify_result = verify_identity_token(attenuated_token.clone(), public_key, base_identity.clone());
        assert!(
        ...
        assert!(
            verify_identity_token(attenuated_token.clone(), public_key, delegated_identity.clone()).is_ok(),
            "Delegated identity should verify"
        );

        // Different branch should fail
        assert!(
            verify_identity_token(attenuated_token.clone(), public_key, "urn:hessra:alice:phone".to_string()).is_err(),
            "Different branch of delegation should fail"
        );

        // Completely different identity should fail
        assert!(
            verify_identity_token(attenuated_token.clone(), public_key, "urn:hessra:bob".to_string()).is_err(),
            "Completely different identity should fail"
        );
    }
    ...
    fn test_multi_level_delegation_chain() {
        ...
        assert!(
            verify_identity_token(token.clone(), public_key, "urn:hessra:company:dept_hr".to_string()).is_err(),
            "Different department should fail"
        );
        assert!(
            verify_identity_token(token.clone(), public_key, "urn:hessra:company:dept_eng:bob".to_string()).is_err(),
            "Different user in same department should fail"
        );
        assert!(
            verify_identity_token(token.clone(), public_key, "urn:hessra:company:dept_eng:alice:phone".to_string()).is_err(),
    ...
    fn test_time_based_expiration() {
        ...
        let expired_token = create_identity_token(
            identity.clone(),
            expired_keypair,
            expired_config,
        )
        ...
        let valid_token = create_identity_token(
            identity.clone(),
            valid_keypair,
            valid_config,
        )
        ...
        assert!(
            verify_identity_token(attenuated_expired, valid_public_key, "urn:hessra:alice:laptop".to_string()).is_err(),
    ...
    fn test_uri_validation_edge_cases() {
        ...
        let test_cases = vec![
            ...
            ("https://example.com/user", "https://example.com/user:device"), // Use : for hierarchy
        ...
        for (base, delegated) in test_cases {
            ...
            assert!(
                ...
                "Base identity {} should NOT verify after delegation", base
            );
            assert!(
                verify_identity_token(attenuated, public_key, delegated.to_string()).is_ok(),
                "Delegated identity {} should verify", delegated
            );
        }
    }
    ...
    fn test_prefix_attack_prevention() {
        ...
        assert!(
            verify_identity_token(alice_token.clone(), public_key, "urn:hessra:alice2".to_string()).is_err(),
        ...
        assert!(
            verify_identity_token(attenuated.clone(), public_key, "urn:hessra:alice:device2".to_string()).is_err(),
            "device2 should not verify against device"
        );
        assert!(
            verify_identity_token(attenuated, public_key, "urn:hessra:alice2:device".to_string()).is_err(),
    ...
    fn test_empty_identity_handling() {
        ...
        let result = create_identity_token(
            "".to_string(),
            keypair,
            TokenTimeConfig::default(),
        );
        ...
        assert!(result.is_ok(), "Should be able to create token with empty identity");
        ...
        assert!(
            verify_identity_token(token.clone(), public_key, "urn:hessra:anyone".to_string()).is_err(),
